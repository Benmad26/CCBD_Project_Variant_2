...
# Dataset sizes to process
SIZES = ["S", "M", "L"]

# Input directory containing the raw Parquet files (generated by dataset_gen.py)
BASE_INPUT_DIR = "data"
# Output directory for the curated layouts
BASE_OUTPUT_DIR = "data/curated"

# --batch-size: rows processed at once (default: 1,000,000)
def parse_args():
    parser = argparse.ArgumentParser()
    parser.add_argument("--batch-size", type=int, default=1_000_000)
    return parser.parse_args()

# Deletes and recreates the curated directory to start fresh
def clean_output():
...
# Derives a date string (YYYY-MM-DD) from the timestamp column
# Used as partition key for the by_date layout
def add_date_column(table):
    date_col = pc.strftime(table["ts"], format="%Y-%m-%d")
    return table.append_column("date", date_col)

# Copies the raw Parquet file into the flat directory without any partitioning
...
# Reads the file batch by batch and writes to by_date and by_region layouts
...
# Creates the 3 layouts (flat, by_date, by_region) for one dataset size
